    ...
    server_1.conn.getConnector().getConnection((err, connection) => {
        if (err) {
            ...
            if (action === "new react") {
                ...
                connection.query(sql, [[[created_at, user_id, comment_id, emoji]]], (err, rows) => {
                    if (err) {
                        return res.status(400).json({
                ...
                connection.query(sql, [created_at, emoji, user_id, comment_id], (err, rows) => {
                    if (err) {
                        return res.status(400).json({
                ...
                connection.query(sql, [user_id, comment_id], (err, rows) => {
                    if (err) {
                        return res.status(400).json({
