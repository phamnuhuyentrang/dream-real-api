...
const getAlbumFavorite = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    server_1.conn.getConnector().getConnection((err, connection) => {
        if (err) {
            ...
            var sql = "SELECT a.id as album_id, a.created_at, a.description, u.first_name, u.last_name, u.avatar, u.id as user_id, g.location_city, g.location_country, g.longitude, g.latitude, t.title, t.slug, a.image, co.comment, re.react, user_react.emoji as user_react FROM album a JOIN favorite f ON a.id = f.album_id LEFT JOIN geo g ON a.geo_id = g.id LEFT JOIN user u ON a.user_id = u.id LEFT JOIN tag t ON a.tag_id = t.id JOIN (SELECT a.id, COUNT(c.id) as comment FROM album a LEFT JOIN comment c ON a.id = c.album_id GROUP BY a.id) co ON a.id = co.id JOIN (SELECT a.id, COUNT(r.id) as react FROM album a LEFT JOIN react r ON a.id = r.album_id GROUP BY a.id) re ON a.id = re.id LEFT JOIN (SELECT r.emoji, a.id FROM user u LEFT JOIN react r ON u.id = r.user_id LEFT JOIN album a ON r.album_id = a.id WHERE u.id = ?) user_react ON user_react.id = a.id WHERE f.user_id = ? LIMIT 10 OFFSET " + offset;
            connection.query(sql, [user_id, user_id], (err, rows) => {
                if (err) {
                    return res.status(400).json({
...
exports.default = getAlbumFavorite;
