...
const getAlbumUser = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    var sql = "WITH fr AS (SELECT u.id, u.first_name, u.last_name, u.avatar, u.cover_image, u.sys_score + u.comm_score AS score, dense_rank() OVER ( ORDER BY u.sys_score + u.comm_score DESC ) AS 'rank' FROM user u) SELECT * FROM fr WHERE fr.id = ? ";
    server_1.conn.getConnector().query(sql, [user_react_id], (err, rows) => {
        if (err) {
            ...
            var sql = "SELECT a.id as album_id, a.created_at, a.description, u.first_name, u.last_name, g.location_city, g.location_country, g.longitude, g.latitude, t.title, t.slug, a.image, co.comment, re.react, user_react.emoji as user_react FROM album a LEFT JOIN geo g ON a.geo_id = g.id LEFT JOIN user u ON a.user_id = u.id LEFT JOIN tag t ON a.tag_id = t.id JOIN (SELECT a.id, COUNT(c.id) as comment FROM album a LEFT JOIN comment c ON a.id = c.album_id GROUP BY a.id) co ON a.id = co.id JOIN (SELECT a.id, COUNT(r.id) as react FROM album a LEFT JOIN react r ON a.id = r.album_id GROUP BY a.id) re ON a.id = re.id LEFT JOIN (SELECT r.emoji, a.id FROM user u LEFT JOIN react r ON u.id = r.user_id LEFT JOIN album a ON r.album_id = a.id WHERE u.id = ?) user_react ON user_react.id = a.id WHERE a.user_id = ? LIMIT 10 OFFSET " + offset;
            server_1.conn.getConnector().query(sql, [user_id, user_react_id], (err, rows) => {
                if (err) {
                    return res.status(200).json({
                        success: false,
                        message: "Error when verifying live location: " + err
